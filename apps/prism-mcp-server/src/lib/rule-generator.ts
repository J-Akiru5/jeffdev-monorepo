...
`;
  ...
  const fullOutput = await generateContent(
    "You are an expert front-end architect. Output rules and skills in clean markdown. Be specific, actionable, and reference exact values from the extraction data.",
    prompt,
  );
...
export function saveRulesLocal(rulesMd: string, skillsMd: string): void {
...
export async function saveRulesToDb(
  rulesMd: string,
  skillsMd: string,
  projectId: string,
  userId: string,
  ...
  // primary key) is generated by Postgres as usual.
