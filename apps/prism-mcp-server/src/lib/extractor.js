...
const MAX_TOKENS_ESTIMATE = (text) => Math.ceil(text.length / 4);
function extractPageData() {
    ...
    allElements.forEach((el) => {
        ...
        if (inline) {
            ...
            if (colorMatch)
                colorMatch.forEach((c) => { const v = c.split(":")[1]; if (v)
                    results.colors.push(v.trim()); });
            const spacingMatch = inline.match(/(?:margin|padding|gap):\s*([^;]+)/gi);
            if (spacingMatch)
                spacingMatch.forEach((s) => { const v = s.split(":")[1]; if (v)
                    results.spacing.push(v.trim()); });
        }
    });
...
export async function scanUrl(url, maxPages = 5, depth = 2) {
    let browser = null;
    ...
    const allComponentPatterns = new Set();
    let totalTokens = 0;
    try {
        ...
        while (pagesToVisit.length > 0 && visited.size < maxPages) {
            ...
            try {
                ...
                extracted.fontSizes.forEach((s) => { if (s)
                    allFontSizes.add(s); });
                extracted.spacing.forEach((s) => { if (s)
                    allSpacing.add(s); });
                ...
                if (current.currentDepth < depth) {
                    const links = await page.evaluate(findInternalLinks, current.url);
                    for (const link of links) {
                        if (!visited.has(link))
                            pagesToVisit.push({ url: link, currentDepth: current.currentDepth + 1 });
                    }
                }
            }
            catch {
...
export function formatExtractionAsMarkdown(tokens) {
    const lines = [
    ...
    if (tokens.componentPatterns.length > 0) {
        ...
        for (const p of tokens.componentPatterns)
            lines.push(`- \`${p}\``);
        lines.push(``);
    }
    return lines.join("\n");
