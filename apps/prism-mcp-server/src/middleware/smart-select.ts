...
const embeddingCache = new Map<
  string,
  { embedding: number[]; updatedAt: string }
>();
...
function summarizeContent(
  content: string,
  maxSentences: number = SUMMARY_SENTENCES,
): string {
...
export function deduplicateRules(rules: RankedRule[]): {
  rules: RankedRule[];
  dedupedCount: number;
} {
  ...
  for (const rule of rules) {
    ...
    for (const existing of deduped) {
      ...
      if (
        ...
        const shorter =
          normalized.length <= existingNorm.length ? rule : existing;
        const longer =
          normalized.length > existingNorm.length ? rule : existing;
...
function truncateByPriority(
  ...
  const medium = rules.filter(
    (r) => r.priority !== undefined && r.priority > 3 && r.priority <= 7,
  );
...
export async function rankRulesByTask(
  ...
  if (ruleDocs.length === 0) {
    return {
      ...
      dedupedRules: 0,
      totalRules: rules.length,
      tokenCount: skills.reduce((s, sk) => s + sk.tokenCount, 0),
    };
  }
  ...
  if (rulesNeedingEmbedding.length > 0) {
    const texts = rulesNeedingEmbedding.map(
      (r) => `${r.rule.name}\n${r.rule.content}`,
    );
    ...
    for (let j = 0; j < rulesNeedingEmbedding.length; j++) {
      ...
      embeddingCache.set(entry.rule._id.toString(), {
        embedding: emb,
  ...
  const rulesTokenCount = kept.reduce(
    (sum, r) => sum + countTokensLocal(r.content),
    0,
  );
...
export function formatRulesResponse(
  ...
  if (result.skills.length > 0) {
    ...
    lines.push(``);
    lines.push(
      `> Skills are procedural guides available on demand. Call \`get_skill\` with a skill ID to load full content.`,
    );
    lines.push(``);
