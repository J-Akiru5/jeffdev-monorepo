...
describe("Database Integration", () => {
  ...
  describe.skipIf(!MONGODB_URI)("Database Operations", () => {
    ...
    it("should find transcript by full-text search", async () => {
      ...
      console.log(
        `✅ Found ${results.length} transcript(s) matching "authentication"`,
      );
    });
    ...
    it("should search by project ID", async () => {
      ...
      console.log(
        `✅ Found ${results.length} transcript(s) in project "test-project"`,
      );
    });
    ...
    it("should return empty array for non-existent search", async () => {
      ...
      const results = await collection
        .find({
          transcriptText: { $regex: "zzz-nonexistent-xyz", $options: "i" },
        })
        .toArray();
