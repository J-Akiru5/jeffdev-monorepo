...
# 2. Check if prism-engine or its workspace dependencies are affected using turbo's built-in filter
#    (replaces deprecated turbo-ignore package)
echo "Checking for changes in apps/prism-engine and its workspace dependencies..."
...
OUTPUT=$(npx -y turbo@^2.7.2 run build --filter="prism-engine...[${BASE}]" --dry=json 2>&1) || {
  echo "⚠️  Turbo check failed — proceeding with build to be safe."
  exit 1;
}

if echo "$OUTPUT" | grep -q '"prism-engine"'; then
  echo "✅ Proceeding: Changes detected in apps/prism-engine or its dependencies."
  exit 1;
else
  echo "🛑 Skipped: No changes detected in apps/prism-engine or its dependencies."
  exit 0;
fi
