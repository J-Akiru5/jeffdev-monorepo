...
export default function AnalyticsPage() {
  ...
  const formatLimit = (v: number | string) => v === -1 || v === "unlimited" ? "∞" : String(v);
  const pct = (used: number, limit: number | string) => {
    if (limit === -1 || limit === "unlimited") return 0;
    return Math.min(100, (used / Number(limit)) * 100);
  };

  const tierColor = tier === "enterprise" ? "text-amber-400"
    : tier === "team" ? "text-purple-400"
    ...
    <div className="space-y-8">
          ...
          <div>
            ...
            <p className="text-white/60 mt-1">
              ...
              <span className="font-mono text-xs text-white/30">
                Resets {data?.resetDate ? new Date(data.resetDate).toLocaleDateString() : "monthly"}
              </span>
            </p>
          </div>
          <span className={`font-mono text-sm font-semibold uppercase ${tierColor} border border-current/30 rounded-full px-3 py-1`}>
            {tier} plan
          </span>
        </div>
      </div>
      ...
      {error && !loading && (
        <GlassPanel className="p-6 border-red-500/20">
          ...
          <button onClick={load} className="mt-2 text-xs text-white/50 hover:text-white underline">
            Retry
          </button>
        </GlassPanel>
      )}
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <GlassPanel className="p-6">
              ...
              <div className="mt-6 space-y-5">
                {([
                  { label: "Projects", key: "projects", color: "from-cyan-500 to-blue-500" },
                  { label: "Rules", key: "rules", color: "from-purple-500 to-pink-500" },
                  { label: "AI Generations", key: "aiGenerations", color: "from-amber-500 to-orange-500" },
                  { label: "Components", key: "components", color: "from-emerald-500 to-teal-500" },
                    ...
                    <div key={key}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-white">{u.used}</span>
                          <span className="text-xs text-white/30">/ {formatLimit(u.limit)}</span>
                        </div>
                      </div>
            ...
            <GlassPanel className="p-6">
              ...
              <div className="mt-6">
                {tel?.tokensByTool && Object.keys(tel.tokensByTool).length > 0 ? (
                    ...
                    {Object.entries(tel.tokensByTool)
                      ...
                      .map(([tool, tokens]) => {
                        ...
                        return (
                          <div key={tool}>
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-mono text-white/60 truncate max-w-[180px]">{tool}</span>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-sm text-white/30">No MCP calls yet this month.</p>
                    <p className="text-xs text-white/20 mt-1">
                      Connect your IDE and run <code className="font-mono">prism serve</code>.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <GlassPanel className="p-6">
              ...
              <div className="mt-6">
                {tel?.callsByPlatform && Object.keys(tel.callsByPlatform).length > 0 ? (
                  <div className="space-y-2">
                    {Object.entries(tel.callsByPlatform)
                      ...
                      .map(([platform, calls]) => {
                        ...
                        const label = platform === "claude_desktop" ? "Claude Desktop"
                        ...
                        return (
                          <div key={platform} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-xs font-mono text-white/40 w-8 text-right">{calls}</span>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-sm text-white/30">No IDE connections detected.</p>
                    <Link href="/quickstart" className="text-xs text-cyan-400 mt-1 hover:underline">
                      Connect your IDE →
                    </Link>
                  </div>
                )}
              ...
              <div className="mt-6 space-y-4">
                <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02] border border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-white/70">Successful Calls</span>
                  </div>
                  <span className="font-mono font-semibold text-emerald-400">
                    {((tel?.totalCalls ?? 0) - (tel?.errorCalls ?? 0)).toLocaleString()}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02] border border-white/5">
                  <div className="flex items-center gap-3">
                  ...
                  <span className={`font-mono font-semibold ${(tel?.errorCalls ?? 0) > 0 ? "text-red-400" : "text-white/30"}`}>
          ...
          {tier === "free" && (
            <GlassPanel className="p-6 border-cyan-500/20 bg-gradient-to-r from-cyan-500/5 to-transparent">
              <div className="flex items-center justify-between gap-6">
                <div>
                  <p className="font-medium text-white">Unlock Full MCP Analytics</p>
                  <p className="text-sm text-white/50 mt-1">
                    Pro plan gives you per-project token breakdowns, 90-day history, and cost alerts.
                  </p>
                </div>
                <Link
                  href="/subscription"
