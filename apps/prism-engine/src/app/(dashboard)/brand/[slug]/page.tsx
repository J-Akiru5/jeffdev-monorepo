...
export default async function BrandPage({ params }: Props) {
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {voice.keywords.map((k, i) => (
                <span
                  key={i}
                  className="text-sm bg-white/5 px-3 py-1 rounded-full text-white/70"
                >
                  {k}
                </span>
              ))}
      ...
      >
        <h2 className="text-sm font-medium mb-4" style={{ color: colors.text }}>
          👁️ Live Preview
        </h2>
        <div
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute right-0 top-full mt-2 w-48 rounded-md border border-white/10 bg-[#0a0a0a] p-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-xl">
        ...
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=cursor`}
          label=".cursorrules"
        />
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=windsurf`}
          label=".windsurfrules"
        />
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=vscode`}
          label="VS Code Settings"
        />
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=claude`}
          label="CLAUDE.md"
        />
        ...
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=css`}
          label="CSS Variables"
        />
        <ExportLink
          href={`/api/brand/export?slug=${slug}&format=tailwind`}
