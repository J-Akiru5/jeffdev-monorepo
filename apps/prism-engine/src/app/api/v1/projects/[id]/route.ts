...
const UpdateProjectSchema = z.object({
  ...
  designSystem: z.enum(['jdstudio', 'bare-minimum', 'glassmorphic', '8bit-nostalgia', 'keandrew', 'custom']).optional(),
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const projects = await getCollection('projects');
  const existing = await projects.findOne({ _id: new ObjectId(id), userId: auth.userId });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const projectId = project._id.toString();
  const [rulesColl] = await Promise.all([
    getCollection('rules'),
  ]);
  await Promise.all([
