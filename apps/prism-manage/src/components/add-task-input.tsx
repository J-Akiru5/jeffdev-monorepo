  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="flex w-full items-center gap-3 rounded-lg border border-dashed border-border p-3 text-sm text-white/40 transition-all hover:border-cyan-500/30 hover:text-cyan-400"
      >
    ...
    <div className="overflow-hidden rounded-lg border border-cyan-500/30 glass-subtle">
      <div className="flex items-center gap-2 px-4">
        ...
        <button
          ...
          className="flex-shrink-0 rounded-md p-1.5 text-white/30 transition-colors hover:bg-white/[0.04] hover:text-cyan-400 disabled:cursor-not-allowed disabled:opacity-30"
        >
          <Maximize2 className="h-4 w-4" />
        </button>
      </div>

      <div className="flex items-center justify-end gap-2 border-t border-white/[0.06] glass-subtle px-3 py-2">
        <button
          onClick={() => {
            setValue("");
            setIsExpanded(false);
          }}
          className="rounded-md px-3 py-1.5 text-xs text-white/50 hover:bg-white/[0.04] hover:text-white"
        >
          Cancel
        </button>
        <button
