...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return errorResponse('Name is required (max 50 chars)', 422);

  const limits = TIER_LIMITS[auth.tier as keyof typeof TIER_LIMITS] || TIER_LIMITS.free;
  if (limits.apiKeys === 0) return errorResponse('API keys not available on your plan', 403);

  const apiKeys = await getCollection('apiKeys');
  const existingCount = await apiKeys.countDocuments({ userId: auth.userId, revokedAt: { $exists: false } });

  if (limits.apiKeys !== -1 && existingCount >= limits.apiKeys) {
    return errorResponse(`API key limit reached (${limits.apiKeys}). Revoke an existing key or upgrade.`, 403);
  }
