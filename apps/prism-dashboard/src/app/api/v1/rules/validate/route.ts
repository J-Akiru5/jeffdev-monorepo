...
export async function POST(request: NextRequest) {
  ...
  const violations: Array<{ rule: string; severity: string; message: string; line?: number }> = [];
  ...
  for (const rule of patternRules) {
    ...
    try {
      ...
      while ((match = regex.exec(code)) !== null) {
        ...
        const snippet = match[0].length > 80 ? match[0].substring(0, 80) + '...' : match[0];
        violations.push({
  ...
  if (patternRules.length === 0) {
    if (code.includes('../../apps/') || code.includes('../apps/')) {
      violations.push({
        ...
        severity: 'error',
        message: 'Never import from `../../apps/*`. Use shared packages like `@repo/*` instead.',
      });
    }
  ...
  const response = successResponse({ violations, count: violations.length, context });
  Object.entries(getRateLimitHeaders(`validate:${auth.userId}`, auth.tier)).forEach(([k, v]) => response.headers.set(k, v));
  return response;
}
