  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-white/50">{invoices?.length || 0} total invoices</p>
      </div>
        ...
        {invoices && invoices.length > 0 ? invoices.map((inv) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-mono text-cyan-400">{inv.invoice_number}</p>
                <p className="text-sm text-white mt-0.5">{inv.amount > 0 ? `$${Number(inv.amount).toLocaleString()}` : "No amount"}</p>
              </div>
              <span
