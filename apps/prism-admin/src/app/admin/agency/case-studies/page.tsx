  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-white/50">{caseStudies?.length || 0} total</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        {caseStudies && caseStudies.length > 0 ? caseStudies.map((cs) => (
          <div
            key={cs.id}
          ...
          >
            ...
            {cs.description && (
              <p className="mt-2 text-xs text-white/40 line-clamp-2">{cs.description}</p>
            )}
            <div className="mt-3 flex items-center gap-2">
              ...
              {cs.slug && (
                <span className="text-[10px] text-white/30 font-mono">{cs.slug}</span>
              )}
            </div>
          </div>
        )) : (
          <div className="col-span-2 py-12 text-center text-white/30">No case studies yet</div>
        )}
      </div>
    </div>
  );
