...
1. Create or generate rules (see [Creating Rules](/en-GB/rules/creating-rules))
...
Navigate to [prism.jeffdev.studio/settings](https://prism.jeffdev.studio/settings)
...
[Full Cursor Setup Guide →](/en-GB/integrations/cursor)
...
[Full Windsurf Setup Guide →](/en-GB/integrations/windsurf)

### VS Code

1. Install the **MCP Extension** from marketplace
2. Open Command Palette → "MCP: Configure Servers"
3. Add Prism server configuration with environment variables

[Full VS Code Setup Guide →](/en-GB/integrations/vscode)
...
[Full Claude Desktop Setup Guide →](/en-GB/integrations/claude-desktop)
...
| `PRISM_API_URL` | No | Dashboard URL (default: `https://prism.jeffdev.studio`) |
...
<Callout type="error">
  **Connection Failed?** See [MCP Setup Troubleshooting](/en-GB/troubleshooting/mcp-setup)
</Callout>
...
- [Quick Start](/en-GB/quick-start) — Create your first rule
- [IDE Setup Guides](/en-GB/integrations/cursor) — Detailed configuration
- [Video Processing](/en-GB/video/upload) — Extract rules from recordings
