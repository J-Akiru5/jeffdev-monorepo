# Installation

Get Prism Context Engine set up in your development environment.

> **Fastest Path**: Use the [Web Dashboard](https://prism.jeffdev.studio) — no installation required for basic usage.

## Choose Your Setup

Prism offers two ways to use context rules:

| Method | Best For | Setup Time |
|--------|----------|------------|
| **Web Dashboard Only** | Quick exports, manual workflow | 2 minutes |
| **MCP Server** | Real-time sync, automated workflow | 10 minutes |

---

## Option A: Web Dashboard (Recommended)

The simplest way to use Prism. No local installation required.

### Step 1: Create an Account

1. Visit [prism.jeffdev.studio/sign-up](https://prism.jeffdev.studio/sign-up)
2. Sign up with your email or GitHub
3. Verify your email address

### Step 2: Create Your First Project

1. Click **"New Project"** in the dashboard
2. Enter your project name (e.g., "My SaaS App")
3. Select your tech stack (Next.js, React, etc.)

### Step 3: Export Rules

1. Create or generate rules (see [Creating Rules](/nl/rules/creating-rules))
2. Click **"Export"** → Choose your IDE format
3. Download the `.cursorrules` or `.windsurfrules` file
4. Place it in your project root

> ⚠️ **Manual Workflow**: With this method, you'll need to re-export rules whenever they change.

---

## Option B: MCP Server (Advanced)

For real-time synchronization between the dashboard and your IDE.

### Prerequisites

- Node.js 18+ installed
- npm or pnpm package manager
- Prism account with API key (Pro plan or above)

### Installation

**npm:**
```bash
npm install -g prism-mcp-server
```

**pnpm:**
```bash
pnpm add -g prism-mcp-server
```

**yarn:**
```bash
yarn global add prism-mcp-server
```

### Get Your API Key

#### Step 1: Open Dashboard Settings

Navigate to [prism.jeffdev.studio/settings](https://prism.jeffdev.studio/settings)

#### Step 2: Generate API Key

1. Scroll to the **"API Access"** section
2. Click **"Generate Key"**
3. Give it a name (e.g., "MacBook Pro")
4. Copy the key immediately — it won't be shown again

#### Step 3: Store Securely

Save your API key somewhere safe. You'll need it for IDE configuration.

### Verify Installation

```bash
prism-mcp --version
# Output: prism-mcp v1.0.3
```

---

## IDE Configuration

After installing the MCP server, configure your editor:

### Cursor

Add to `~/.cursor/mcp.json`:

```json
{
  "mcpServers": {
    "prism": {
      "command": "prism-mcp",
      "env": {
        "PRISM_API_KEY": "prism_live_your_api_key_here",
        "MONGODB_URI": "your_cosmos_connection_string"
      }
    }
  }
}
```

[Full Cursor Setup Guide →](/nl/integrations/cursor)

### Windsurf

Add to `~/.codeium/windsurf/mcp_config.json`:

```json
{
  "mcpServers": {
    "prism": {
      "command": "prism-mcp",
      "env": {
        "PRISM_API_KEY": "prism_live_your_api_key_here",
        "MONGODB_URI": "your_cosmos_connection_string"
      }
    }
  }
}
```

[Full Windsurf Setup Guide →](/nl/integrations/windsurf)

### VS Code

1. Install the **MCP Extension** from marketplace
2. Open Command Palette → "MCP: Configure Servers"
3. Add Prism server configuration with environment variables

[Full VS Code Setup Guide →](/nl/integrations/vscode)

### Claude Desktop

Add to `~/Library/Application Support/Claude/claude_desktop_config.json` (macOS):

```json
{
  "mcpServers": {
    "prism": {
      "command": "prism-mcp",
      "env": {
        "PRISM_API_KEY": "prism_live_your_api_key_here",
        "MONGODB_URI": "your_cosmos_connection_string"
      }
    }
  }
}
```

[Full Claude Desktop Setup Guide →](/nl/integrations/claude-desktop)

---

## Environment Variables

The MCP server uses these environment variables:

| Variable | Required | Description |
|----------|----------|-------------|
| `PRISM_API_KEY` | Yes (Pro+) | Your API key from the dashboard |
| `MONGODB_URI` | Yes | Azure Cosmos DB connection string |
| `COSMOS_DATABASE_NAME` | No | Database name (default: `prism`) |
| `PRISM_API_URL` | No | Dashboard URL (default: `https://prism.jeffdev.studio`) |

---

## Verify Connection

After configuration, verify your MCP connection:

1. Open your IDE
2. Start a new chat with AI
3. Ask: "What Prism rules are available?"

If configured correctly, the AI will list your project's context rules.

---

## Troubleshooting

<Callout type="error">
  **Connection Failed?** See [MCP Setup Troubleshooting](/nl/troubleshooting/mcp-setup)
</Callout>

### Common Issues

| Issue | Solution |
|-------|----------|
| `command not found: prism-mcp` | Reinstall with `-g` flag or check PATH |
| `Invalid API key` | Regenerate key in dashboard |
| `Connection timeout` | Check internet connection |
| `No rules found` | Create rules in dashboard first |
| `API key validation failed` | Ensure you have a Pro plan or above |

---

## Next Steps

- [Quick Start](/nl/quick-start) — Create your first rule
- [IDE Setup Guides](/nl/integrations/cursor) — Detailed configuration
- [Video Processing](/nl/video/upload) — Extract rules from recordings
