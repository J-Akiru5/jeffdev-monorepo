  ...
  const columns: ColumnDef<UserProfile>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span
            className={`inline-flex items-center gap-1 rounded-sm border px-2 py-0.5 text-[10px] uppercase tracking-wider ${config.className}`}
          >
            ...
            {activeMenu === user.uid && (
              <>
                ...
                <div className="absolute right-0 top-full z-20 mt-1 min-w-[160px] rounded-md border border-white/10 bg-[#0a0a0a] py-1 shadow-xl">
                  {/* Role options */}
                  {(["admin", "partner", "employee"] as UserRole[]).map(
                    (role) => (
                      <button
                        key={role}
