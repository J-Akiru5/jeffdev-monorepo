...
_Goal: Documentation is not an afterthought; it is a compilation requirement._

- **Rule:** If a feature is shipped without a corresponding update to `apps/prism-docs` or a TSDoc comment, the task is **INCOMPLETE**.
- **The "Why" Filter:** Do not document _what_ the code does (the code shows that). Document _why_ it exists and _how_ it connects to the Business Brain.

## 2. IN-CODE ANNOTATIONS (TSDOC)

_We write code for the "Next Developer" (who is likely us in 6 months)._
...
const UserSchema = z.object({
  ...
  email: z
    .string()
    .email()
    .describe("Must be a valid corporate email. No public domains in Admin."),
});
```

## 3. PUBLIC DOCUMENTATION (`apps/prism-docs`)

_Powered by Nextra. This is the "User Manual" for Prism._
...
_We follow "Keep a Changelog" standards._
