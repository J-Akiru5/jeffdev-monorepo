...
_Goal: Harden the ecosystem against leaks, injection, and unauthorized access._
...
- _Local Dev:_ `doppler run -- turbo dev`
- _CI/CD:_ Use the Doppler Integration token in GitHub Actions.
...
_Goal: Interaction to Next Paint (INP) < 200ms. The app must "feel" native._
...
- _User clicks "Save Rule"_ → _UI instantly shows "Rule Saved"_ → _Background syncs to Azure_.
...
- _Why:_ They handle focus management and keyboard navigation off the main thread better than custom React `useEffect` spaghetti.
...
- _Limit:_ 10 requests / 10 seconds per IP for API routes.

- **Auth Guard:** Middleware must verify the session (Firebase or Clerk) _before_ the request hits the layout.
