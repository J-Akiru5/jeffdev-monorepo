...
export async function listMarketplace(options: MarketplaceOptions) {
  ...
  if (items.length === 0) {
    console.log(chalk.yellow("No marketplace rule sets found."));
    return;
  }
  ...
  for (const rs of items) {
    console.log(
      `  ${chalk.bold(rs.name)}  [${chalk.dim(`${rs.ruleCount} rules`)}]`,
    );
...
export async function installMarketplace(
  ...
  const result = await apiFetch(`/api/v1/marketplace/install/${id}`, {
    method: "POST",
  });
  if (result.error) {
  ...
  console.log(
    `  ${chalk.bold(data.ruleSetName)}: ${data.installed} of ${data.totalRules} rules installed`,
  );
}
