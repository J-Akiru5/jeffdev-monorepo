...
export async function listProjects(options: ProjectsOptions) {
  ...
  for (const p of items) {
    console.log(
      `  ${chalk.bold(p.name)}  [${chalk.dim(p.stack)} / ${chalk.dim(p.designSystem)}]`,
    );
    console.log(
      `    ${chalk.dim("slug:")} ${p.slug}  ${chalk.dim("rules:")} ${p.ruleCount}  ${chalk.dim("videos:")} ${p.videoCount}`,
    );
  }
  console.log("");
}
...
export async function viewProject(slug: string, options: { json?: boolean }) {
  const projects =
    await apiFetch<Array<{ id: string; slug: string; name: string }>>(
      "/api/v1/projects",
    );
  ...
  const rulesRes = await apiFetch<Array<Record<string, unknown>>>(
    `/api/v1/projects/${project.id}/rules`,
  );
...
export async function createProject(options: {
  ...
  console.log(
    `  ${chalk.bold(p.name)} (${p.slug}) [${p.stack} / ${p.designSystem}]`,
  );
}
...
export async function deleteProject(
  ...
  if (!options.force && !options.json) {
    const answer = await prompt(
      chalk.red(`Delete project "${slug}"? This cannot be undone. (y/N): `),
    );
