...
import { scanRepo, formatScanReport, type RepoScanReport } from "./repo-scanner.js";
...
describe("scanRepo", () => {
  it("scans a simple directory structure", async () => {
    ...
    writeFileSync(join(tmp, "src", "index.ts"), `import { foo } from "./lib/bar";\nconst x = 1;\n`);
    writeFileSync(join(tmp, "src", "components", "Button.tsx"), `import React from "react";\nexport function Button() { return <button />; }`);
    writeFileSync(join(tmp, "src", "lib", "utils.ts"), `export function formatDate(d: Date) { return d.toISOString(); }`);
    writeFileSync(join(tmp, "package.json"), JSON.stringify({ name: "test", dependencies: { react: "^18" } }));
    ...
    expect(Object.keys(report.namingConventions.files).length).toBeGreaterThan(0);
  });
  ...
  it("detects naming conventions", async () => {
    ...
    writeFileSync(join(tmp, "src", "PascalComponent.tsx"), `export const MyComponent = () => null;`);
  ...
  it("detects import patterns", async () => {
    ...
    writeFileSync(join(tmp, "src", "app.tsx"), [
...
describe("formatScanReport", () => {
  it("formats a report into markdown", () => {
    const report: RepoScanReport = {
      root: "/test",
      namingConventions: { files: { camelCase: 5, PascalCase: 3 }, functions: {}, components: {}, variables: { camelCase: 10 } },
      imports: { relative: 20, absolute: 30, external: { react: 5 }, internal: { "@repo/ui": 3 } },
      structure: { directories: ["src", "src/components"], fileCount: 15, dirCount: 5, languages: { ".ts": 10, ".tsx": 5 } },
