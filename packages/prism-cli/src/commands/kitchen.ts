...
export async function kitchenAnalyze(options: AnalyzeOptions): Promise<void> {
  ...
  const ruleSections = rulesMd.split(/(?=##|\*\*)/).filter((s) => s.trim().length > 0);
  const skillSections = skillsMd.split(/(?=##|\*\*)/).filter((s) => s.trim().length > 0);
  ...
  if (taskWords.length > 0) {
    for (const section of ruleSections) {
      ...
      const matchCount = taskWords.filter((w) => sectionLower.includes(w)).length;
  ...
  const skippedCount = Math.max(0, allRelevant.length - Math.floor(budget / 100));
  ...
  console.log(`  ${chalk.bold("Sections:")}  ${ruleSections.length + skillSections.length} total`);
  if (task) console.log(`  ${chalk.bold("Relevant:")}  ${allRelevant.length} (from keyword match)`);
  console.log(`  ${chalk.bold("Keep:")}      ${kept.length} full sections`);
  if (summarized.length > 0) console.log(`  ${chalk.bold("Summarize:")} ${summarized.length} sections (truncated to ~50 tokens)`);
  if (skippedCount > 0) console.log(`  ${chalk.bold("Skipped:")}   ${skippedCount} sections (over token budget)`);
  console.log(`  ${chalk.bold("Tokens:")}    ${totalTokens} total, ~${budget - runningBudget} after trim (budget: ${budget})`);
  console.log(`  ${chalk.bold("Budget:")}    ${budget < totalTokens ? chalk.yellow(`${budget} (over by ${totalTokens - budget} tokens)`) : chalk.green(`${budget} (within limit)`)}`);
  console.log("");
}
...
export async function kitchenPreview(options: PreviewOptions): Promise<void> {
  ...
  if (rulesMd) {
    ...
    if (task) {
      ...
      if (!lines.some((l) => taskWords.some((w) => l.toLowerCase().includes(w)))) {
        console.log(`  ${chalk.dim("(no matching rules — showing all)")}`);
        lines.forEach((l) => console.log(`  ${l}`));
      }
    } else {
      lines.slice(0, 40).forEach((l) => console.log(`  ${l}`));
      if (lines.length > 40) console.log(`  ${chalk.dim(`... ${lines.length - 40} more lines`)}`);
    }
    console.log("");
  }
  ...
  console.log(chalk.dim(`  Total: ~${totalTokens} tokens (${rulesMd.length + skillsMd.length} chars)`));
  console.log("");
}
...
export async function kitchenTrim(options: TrimOptions): Promise<void> {
  ...
  const sections = rulesMd.split(/(?=##|\*\*)/).filter((s) => s.trim().length > 0);
  const tokenCounts = sections.map((s) => ({ section: s, tokens: countTokens(s) }));
  ...
  if (options.json) {
    console.log(JSON.stringify({
      ...
      keptTokens: running,
      budget,
      removedTitles: removed.map((r) => r.split("\n")[0]?.replace(/[#*]/g, "").trim()).filter(Boolean),
    }, null, 2));
    return;
  }
  ...
  console.log(`  ${chalk.bold("Original:")}   ${sections.length} sections, ~${originalTokens} tokens`);
  console.log(`  ${chalk.bold("After:")}      ${kept.length} sections, ~${running} tokens`);
...
export async function kitchenHistory(options: { json?: boolean }): Promise<void> {
  ...
  for (const s of last10) {
    ...
    const reductionStr = s.reduction > 0 ? chalk.green(`-${s.reduction}%`) : chalk.dim("baseline");
    console.log(`  ${chalk.bold(s.task || "(no task)")}`);
    console.log(`    ${date}  |  Est: ${s.estimatedTokens} tokens  |  ${reductionStr}`);
    console.log("");
  }

  const avgReduction = sessions.reduce((sum, s) => sum + (s.reduction || 0), 0) / sessions.length;
  console.log(chalk.dim(`  Avg reduction: ${avgReduction.toFixed(1)}% across ${sessions.length} sessions`));
  console.log("");
}

export async function kitchenOptimize(options: { json?: boolean }): Promise<void> {
  ...
  const sections = rulesMd.split(/(?=##|\*\*)/).filter((s) => s.trim().length > 0);
  const toolCalls = events.filter((e) => e.toolName === "get_architectural_rules").length;
  ...
  console.log(`  ${chalk.bold("Rules:")}       ${sections.length} sections in ~/.prism/rules.md`);
  console.log(`  ${chalk.bold("Telemetry:")}   ${totalEvents} events (${toolCalls} get_architectural_rules calls)`);
  console.log(`  ${chalk.bold("Status:")}      ${chalk.green("Ready for optimization")}`);
  ...
  if (sections.length > 0) {
    ...
    ranked.slice(0, mid).forEach((r) => console.log(chalk.green(`    ↑ ${r.title} (${r.tokens} tok) — promote`)));
    ranked.slice(mid).forEach((r) => console.log(chalk.dim(`    ↓ ${r.title} (${r.tokens} tok) — demote`)));
  }
