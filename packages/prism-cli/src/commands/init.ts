...
const VSCODE_USER_DIR = process.platform === 'win32'
...
async function setupCursor(): Promise<boolean> {
  ...
  const mcpServers = (mcpConfig.mcpServers as Record<string, McpServerConfig>) || {};
...
async function setupVSCode(): Promise<boolean> {
  ...
  console.log('  ℹ Install Prism extension from VS Code marketplace for full support.');
  return true;
}
