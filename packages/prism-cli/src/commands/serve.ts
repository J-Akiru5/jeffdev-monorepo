...
const MCP_SERVER_SCRIPT = join(
  import.meta.dirname, '..', '..', '..', '..',
  'apps', 'prism-mcp-server', 'src', 'index.ts',
);

// Try to find the built JS version first
const MCP_SERVER_BUILT = join(
  import.meta.dirname, '..', '..', '..', '..',
  'apps', 'prism-mcp-server', 'dist', 'index.js',
);
...
export async function serve(options: ServeOptions): Promise<void> {
  ...
  console.error(chalk.dim(`  Env: ${process.env.MONGODB_URI ? 'DB configured' : 'DB not configured (offline fallback)'}`));
  ...
  if (!options.offline) {
    ...
    console.error(chalk.yellow('  ⚠ Full MCP server unavailable, falling back to local cache.'));
...
async function tryFullServer(): Promise<ChildProcess | null> {
  const scriptPath = existsSync(MCP_SERVER_BUILT) ? MCP_SERVER_BUILT : MCP_SERVER_SCRIPT;
  ...
  if (scriptPath.endsWith('.ts')) {
    ...
    // Find tsx CLI
    const tsxPaths = [
      join(import.meta.dirname, '..', '..', '..', '..', 'node_modules', '.pnpm', 'tsx@4.21.0', 'node_modules', 'tsx', 'dist', 'cli.mjs'),
      join(import.meta.dirname, '..', '..', '..', '..', 'node_modules', 'tsx', 'dist', 'cli.mjs'),
    ];
    const tsxPath = tsxPaths.find(p => existsSync(p));
    if (!tsxPath) {
      console.error(chalk.dim('  tsx runtime not found — MCP server requires build. Run `pnpm build`'));
      return null;
    }
  ...
  try {
    ...
    const child = spawn(cmd, args, {
      ...
      env: { ...process.env },
      cwd: join(import.meta.dirname, '..', '..', '..', '..', 'apps', 'prism-mcp-server'),
    });
    ...
    if (!started) {
      ...
      if (child.exitCode === null || child.exitCode === 0) {
        console.error(chalk.dim('  MCP server starting (waiting for handshake)...'));
      } else {
        console.error(chalk.dim(`  MCP server exited with code ${child.exitCode}`));
        return null;
      }
    }
    ...
    console.error(chalk.dim(`  Failed to start full MCP server: ${e instanceof Error ? e.message : e}`));
    return null;
  }
}
...
async function runLiteServer(): Promise<void> {
  ...
  if (existsSync(RULES_CACHE)) {
    try {
      ...
      console.error(chalk.yellow('  ⚠ Cache corrupted, starting with empty rule set.'));
    }
  }
    ...
    tools: [
      {
        ...
        description: 'Fetch critical coding standards and design rules. Provide a "task" for relevance ranking.',
        inputSchema: {
          type: 'object' as const,
          properties: {
            task: { type: 'string', description: 'Describe what you\'re about to code' },
            maxTokens: { type: 'number', description: 'Max tokens (default 4000)', default: 4000 },
      ...
      {
        name: 'get_skill',
        description: 'Fetch full skill content (procedural guide with code examples) by ID or name.',
        inputSchema: {
          type: 'object' as const,
          properties: { skillId: { type: 'string', description: 'Skill ID or name' } },
          required: ['skillId'],
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            violation: {
              ...
              properties: {
                ruleId: { type: 'string' }, ruleName: { type: 'string' },
                pattern: { type: 'string' }, message: { type: 'string' },
                severity: { type: 'string' }, line: { type: 'number' },
                column: { type: 'number' }, endLine: { type: 'number' },
                endColumn: { type: 'number' }, matchedText: { type: 'string' },
      ...
      {
        name: 'repo_extract',
        description: 'Analyze a repository scan report and generate architectural governance rules via AI.',
        inputSchema: {
          type: 'object' as const,
          properties: {
            scan: {
      ...
      },
      {
        ...
        description: 'Check if code follows project architectural rules (legacy pattern-based check).',
        inputSchema: {
          type: 'object' as const,
          properties: {
    ...
    switch (name) {
      case 'get_architectural_rules': {
        ...
        if (filtered.length === 0) {
          return { content: [{ type: 'text' as const, text: 'No rules found. Run `prism sync` to populate.' }] };
        }
        ...
        const keywords = task ? task.toLowerCase().split(/\s+/).filter(w => w.length > 3) : [];
        ...
        const text = isJson
          ? JSON.stringify({ rules: kept.map((r) => ({ id: r._id || r.id || r.slug, name: r.name, priority: r.priority, category: r.category, content: r.content, score: (r as { score?: number }).score })), meta: { task, returnedRules: kept.length, totalRules: filtered.length, tokenCount: used, provider: detectedClient } })
      ...
      case 'get_skill': {
        ...
        const doc = rules.find((r) => r._id === skillId || r.id === skillId || r.slug === skillId || (r.name || '').toLowerCase().includes(skillId.toLowerCase()));
        if (!doc) return { content: [{ type: 'text', text: `Skill "${skillId}" not found.` }], isError: true };
      ...
      case 'validate_code': {
        ...
        const patternRules = rules.filter((r) => r.pattern && r.isActive !== false);
        if (patternRules.length === 0) {
          return { content: [{ type: 'text', text: JSON.stringify({ status: 'pass', violations: [], checkedRules: 0, note: 'No pattern rules in local cache. Run `prism sync` to populate.' }) }] };
        }
        ...
        return { content: [{ type: 'text', text: JSON.stringify({ status: violations.length === 0 ? 'pass' : 'fail', violations, checkedRules: patternRules.length }) }] };
      }
      ...
      case 'prism_fix': {
        ...
        if (pattern.includes('../../apps/') || pattern.includes('../apps/') || matchedText.includes('../../apps/')) {
        ...
        } else if (pattern.includes('style={') || matchedText.includes('style={{')) {
          correctedCode = fixCode.replace(/style=\{\{[^}]*\}\}/g, '{/* TODO: Replace with Tailwind classes */}');
          confidence = 0.6;
          changes.push({ line: (violation.line as number) || 0, from: matchedText, to: '{/* TODO: Tailwind */}' });
        } else if (pattern.includes('console.') || matchedText.includes('console.')) {
          correctedCode = fixCode.replace(/console\.(log|warn|error|debug|info)\([^)]*\);?/g, (m) => `// ${m.trim()}`);
          confidence = 0.9;
          changes.push({ line: (violation.line as number) || 0, from: matchedText, to: `// ${matchedText}` });
        } else {
        ...
        return { content: [{ type: 'text', text: JSON.stringify({ correctedCode, appliedRule: ruleName, confidence, changes }) }] };
      }

      case 'repo_extract': {
        return { content: [{ type: 'text', text: 'repo_extract requires the full MCP server with AI provider access. Run without --offline.' }], isError: true };
      }

      case 'validate_code_pattern': {
        const code = a.code as string;
        if (!code) return { content: [{ type: 'text', text: 'No code provided.' }], isError: true };
        ...
        for (const rule of patternRules) {
          try {
            if (new RegExp(rule.pattern!, 'gi').test(code)) {
              v.push(`${rule.severity === 'error' ? '❌' : '⚠️'} **${rule.name}**: ${rule.content}`);
            }
          } catch { /* skip */ }
        }
        const text = v.length === 0 ? '✅ No violations detected.' : `# Code Validation Report\n\n${v.join('\n\n')}`;
  ...
  console.error(chalk.green('✓ Prism MCP Server running (lite mode — local cache)'));
