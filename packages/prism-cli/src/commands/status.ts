...
export async function status(options: StatusOptions = {}): Promise<void> {
  ...
  if (existsSync(projectsCache)) {
    try {
      const raw = JSON.parse(readFileSync(projectsCache, 'utf-8')) as
        | Array<{ name: string }>
        | { data?: Array<{ name: string }>; projects?: Array<{ name: string }> };
      const projects = Array.isArray(raw)
        ? raw
        : ((raw as { data?: Array<{ name: string }> }).data ?? (raw as { projects?: Array<{ name: string }> }).projects ?? []);
  ...
  if (existsSync(cacheDir)) {
    try {
      ...
      cacheEntries = readdirSync(cacheDir).filter(f => f.endsWith('.json')).length;
    } catch { /* ignore */ }
  }
  const cacheStatus = cacheEntries > 10 ? 'warm' : cacheEntries > 0 ? 'cold' : 'empty';
  ...
  row('Auth:', opts.token ? chalk.green(`✓ Authenticated (${opts.token.slice(0, 14)}...)`) : chalk.yellow('Not authenticated — run `prism login`'));
  row('Project:', activeProject ? chalk.white(activeProject) : chalk.dim('none'));
  row('Rules:', ruleCount > 0
    ? `${chalk.white(String(ruleCount))} total — ${chalk.cyan(String(highPriority))} high priority`
    : chalk.dim('none — run `prism sync`'));
  row('Last sync:', lastSync === 'never' ? chalk.yellow('never — run `prism sync`') : chalk.white(lastSync));
  row('Cache:', cacheStatus === 'warm'
