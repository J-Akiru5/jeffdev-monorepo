...
export async function sync(options?: { full?: boolean; repo?: string | boolean }): Promise<void> {
  ...
  if (options?.repo) {
    ...
    try {
      ...
      spinner.succeed(`Scanned ${report.structure.fileCount} files in ${report.structure.dirCount} directories`);

      console.log(formatScanReport(report));

      // Save report locally
      if (!existsSync(DATA_DIR)) mkdirSync(DATA_DIR, { recursive: true });
      writeFileSync(join(DATA_DIR, 'repo-scan.json'), JSON.stringify(report, null, 2));
      ...
      if (opts.token) {
        const upload = await promptYesNo('Generate governance rules from this scan?');
        if (upload) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              genSpinner.succeed(`Generated ${result.rulesCreated || 0} rules from repo scan`);
            } else {
              genSpinner.fail('Rule generation failed: ' + (await res.text()).slice(0, 100));
            }
          } catch (e) {
            genSpinner.fail('Could not reach Prism Cloud for AI generation');
            console.log(chalk.dim('   Run without --repo or check your connection.'));
          }
        }
      }
    } catch (error) {
  ...
  const spinner = ora(useDelta ? 'Delta syncing data from Prism Cloud...' : 'Syncing data from Prism Cloud...').start();
  ...
  try {
    ...
    const syncRules = async (res: Response) => {
      ...
      if (useDelta && rules.data?.length === 0) {
        ...
        const ruleList: Array<{ id?: string; name?: string; content?: string; category?: string; priority?: number; tags?: string[]; pattern?: string; severity?: string; isActive?: boolean; skillsContent?: string }> = Array.isArray(rules) ? rules : (rules.data || []);
        ...
        try {
          if (ruleList.length > 0) {
            const mdLines = ['# Prism Architectural Rules', '', `Synced: ${new Date().toISOString().slice(0, 10)}`, ''];
            for (const r of ruleList) {
              ...
              if (r.tags?.length) mdLines.push(`**Tags:** ${r.tags.join(', ')}  `);
              mdLines.push('', r.content || '', '', '---', '');
            }
            writeFileSync(join(DATA_DIR, 'rules.md'), mdLines.join('\n'));
            const rulesCacheDir = join(DATA_DIR, 'rules');
            if (!existsSync(rulesCacheDir)) mkdirSync(rulesCacheDir, { recursive: true });
            writeFileSync(join(rulesCacheDir, 'rules.json'), JSON.stringify(ruleList, null, 2));
          }
        } catch { /* non-critical */ }
      }
    };
    ...
    try {
      ...
      if (rulesRes.ok) {
        ...
        const fallbackRes = await fetch(`${opts.apiUrl}/api/v1/rules?limit=100`, { headers });
    ...
    const deltaNote = useDelta ? ` (delta since ${new Date(lastSyncedAt).toLocaleString()})` : '';
    spinner.succeed(`Synced: ${synced.rules} rules, ${synced.projects} projects, ${synced.brands} brands, ${synced.components} components${deltaNote}`);
