...
export async function listBrands(options: { json?: boolean }) {
  const result = await apiFetch<Array<Record<string, unknown>>>('/api/v1/brands');
  ...
  if (items.length === 0) { console.log(chalk.yellow('No brands found.')); return; }

  console.log(chalk.cyan(`\nBrands (${items.length}):\n`));
  for (const b of items) {
    const colors = b.colors as Record<string, string> | undefined;
    const swatch = colors ? `${colors.primary} ${colors.accent}` : '';
    console.log(`  ${chalk.bold(b.companyName)}  [${chalk.dim(b.industry)}]  ${chalk.dim(swatch)}`);
...
export async function viewBrand(slug: string, options: { json?: boolean }) {
  const brands = await apiFetch<Array<{ id: string; slug: string }>>('/api/v1/brands');
  const brand = (brands.data || []).find(b => b.slug === slug || b.id === slug);
  if (!brand) { console.error(chalk.red('Brand not found.')); process.exit(1); }

  const result = await apiFetch<Record<string, unknown>>(`/api/v1/brands/${brand.id}`);
  if (result.error) { console.error(chalk.red(`Error: ${result.error}`)); process.exit(1); }

  const b = result.data!;
  if (options.json) { console.log(JSON.stringify(b, null, 2)); return; }
  ...
  if (typography) {
    console.log(chalk.bold('\n  Typography:'));
    console.log(`    Heading: ${typography.headingFont}  |  Body: ${typography.bodyFont}`);
  ...
  if (voice) {
    console.log(chalk.bold('\n  Voice:'));
    console.log(`    Personality: ${voice.personality}  |  Formality: ${voice.formality}`);
...
export async function createBrand(options: { json?: boolean }) {
  ...
  const industryAnswer = await prompt('Industry (photography/tech/agency/ecommerce/saas/healthcare/finance/education/other): ');
  ...
  const payload = {
    ...
    typography: { headingFont, bodyFont, monoFont: monoFont || undefined, scale },
    voice: { personality, formality, keywords: keywords.split(',').map(k => k.trim()).filter(Boolean) },
    imagery: { style, mood },
    spacing: { unit, borderRadius },
  };

  const spinner = ora('Creating brand...').start();
  const result = await apiFetch('/api/v1/brands', { method: 'POST', body: payload });
...
export async function exportBrand(slug: string, options: { format: string; json?: boolean; output?: string }) {
  const brands = await apiFetch<Array<{ id: string; slug: string }>>('/api/v1/brands');
  const brand = (brands.data || []).find(b => b.slug === slug || b.id === slug);
  if (!brand) { console.error(chalk.red('Brand not found.')); process.exit(1); }
  ...
  const response = await fetch(`${opts.apiUrl}/api/v1/brands/${brand.id}/export?format=${format}`, {
...
export async function deleteBrand(slug: string, options: { json?: boolean; force?: boolean }) {
  ...
  const result = await apiFetch(`/api/v1/brands/${brand.id}`, { method: 'DELETE' });
  if (result.error) { spinner.fail(result.error); process.exit(1); }
