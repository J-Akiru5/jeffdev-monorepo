...
export async function listRules(options: RulesOptions) {
  ...
  const result = await apiFetch<Array<Record<string, unknown>>>(`/api/v1/rules?${params}`);
  ...
  if (items.length === 0) { console.log(chalk.yellow('No rules found.')); return; }
...
export async function createRule(options: { json?: boolean; project?: string; name?: string; category?: string; content?: string; priority?: number }) {
  if (!options.json) {
    if (!options.name) options.name = await prompt('Rule name: ');
    if (!options.category) {
      console.log('Categories: architecture | styling | security | performance | testing | documentation | custom');
      options.category = await prompt('Category: ');
    }
    if (!options.content) options.content = await prompt('Content: ');

    if (!options.project) {
      const projectSlug = await prompt('Project slug (optional, press enter to skip): ');
      if (projectSlug) options.project = projectSlug;
    }
  }

  if (!options.name || !options.category || !options.content) {
    console.error(chalk.red('Missing required fields: name, category, content'));
    process.exit(1);
  }
  ...
  if (options.project) {
    ...
    const project = (projects.data || []).find(p => p.slug === options.project || p.id === options.project);
...
export async function editRule(id: string, options: { json?: boolean; content?: string; name?: string; category?: string }) {
  const body: Record<string, unknown> = {};
  ...
  const result = await apiFetch(`/api/v1/rules/${id}`, { method: 'PATCH', body });
  if (result.error) { spinner.fail(result.error); process.exit(1); }
...
export async function deleteRule(id: string, options: { json?: boolean; force?: boolean }) {
