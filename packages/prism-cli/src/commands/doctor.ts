...
export async function doctor(options: DoctorOptions = {}): Promise<void> {
  ...
  try {
    ...
    const res = await fetch(`${PRISM_API_URL}/api/health`, {
  ...
  if (claudeConfigPath) {
    ...
    try {
      const cfg = JSON.parse(readFileSync(claudeConfigPath, "utf-8")) as {
        mcpServers?: Record<string, unknown>;
      };
      if (cfg.mcpServers?.prism) configuredIDEs.push("Claude Desktop");
      else missingIDEs.push("Claude Desktop");
    } catch {
      missingIDEs.push("Claude Desktop");
    }
  }

  const ideStatus: CheckStatus =
    detectedIDEs.length === 0
      ? "warn"
      : missingIDEs.length === 0
        ? "pass"
        : configuredIDEs.length > 0
          ? "warn"
          : "fail";
  ...
  results.push({
    ...
    fix:
      ideStatus !== "pass" && detectedIDEs.length > 0
  ...
  if (allGood) {
    ...
    console.log(
      `  ${chalk.bold(`${passes}/${total} checks passed`)} — ${fails > 0 ? chalk.red(`${fails} error${fails !== 1 ? "s" : ""}`) : ""}${fails > 0 && warns > 0 ? ", " : ""}${warns > 0 ? chalk.yellow(`${warns} warning${warns !== 1 ? "s" : ""}`) : ""}`,
    );
    if (results.some((r) => r.status === "fail" || r.status === "warn")) {
      ...
      console.log(
        chalk.dim("  Run `prism sync` to populate the local rules cache."),
      );
    }
  }
  console.log("");
